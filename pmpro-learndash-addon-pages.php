<?php // phpcs:ignore
/**
 * LearnDash + PMPro Addon Pages
 *
 * @package   pmpro
 * @copyright Copyright(c) 2020, MediaRon LLC
 * @license http://opensource.org/licenses/GPL-2.0 GNU General Public License, version 2 (GPL-2.0)
 *
 * Plugin Name: LearnDash + PMPro Addon Pages
 * Plugin URI: https://mediaron.com
 * Description: Integrate LearnDash with PMPro Add-on Pages.
 * Version: 1.0.0
 * Author: MediaRon LLC
 * Author URI: https://mediaron.com
 * License: GPL2
 * License URI: http://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: pmpro-learndash-addon-pages
 * Domain Path: languages
 */

define( 'PMPRO_LD_AOP_VERSION', '1.0.0' );
define( 'PMPRO_LD_AOP_PLUGIN_NAME', 'LearnDash + PMPro Addon Pagesy' );
define( 'PMPRO_LD_AOP_DIR', plugin_dir_path( __FILE__ ) );
define( 'PMPRO_LD_AOP_URL', plugins_url( '/', __FILE__ ) );
define( 'PMPRO_LD_AOP_SLUG', plugin_basename( __FILE__ ) );
define( 'PMPRO_LD_AOP_FILE', __FILE__ );

// Setup the plugin auto loader.
require_once 'autoloader.php';

/**
 * The Voting Tally base class.
 */
class PMPRO_LearnDash_AOP {

	/**
	 * PMPRO_LearnDash_AOP instance.
	 *
	 * @var PMPRO_LearnDash_AOP $instance
	 */
	private static $instance = null;

	/**
	 * Return a class instance.
	 */
	public static function get_instance() {
		if ( null === self::$instance ) {
			self::$instance = new self();
		}
		return self::$instance;
	}

	/**
	 * Class Constructor
	 */
	private function __construct() {
		add_action( 'plugins_loaded', array( $this, 'plugins_loaded' ), 20 );
		add_action( 'init', array( $this, 'init' ) );
	}

	/**
	 * Fired when the init action for WordPress is triggered.
	 */
	public function init() {
		load_plugin_textdomain( 'pmpro-learndash-addon-pages', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	}

	/**
	 * Fired when the plugins for WordPress have finished loading.
	 */
	public function plugins_loaded() {
		if ( ! function_exists( 'pmproap_post_meta' ) || ! function_exists( 'pmpro_has_membership_access' ) || ! function_exists( 'learndash_user_get_enrolled_courses' ) ) {
			return;
		}
		$this->post_meta = new \PMPRO_LearnDash_AOP\Includes\Addon_Pages();
		$this->post_meta->run();

		$this->redirect = new \PMPRO_LearnDash_AOP\Includes\Redirect();
		$this->redirect->run();

		$this->add_remove_courses = new \PMPRO_LearnDash_AOP\Includes\Add_Remove_Courses();
		$this->add_remove_courses->run();
	}
}
PMPRO_LearnDash_AOP::get_instance();
